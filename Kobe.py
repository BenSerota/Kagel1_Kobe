...
for var in date_vars:
	...
	rawdata[var + '_yearday_x'] = np.cos(yearday)
	rawdata[var + '_month_x'] = np.sin(month)
	rawdata[var + '_month_x'] = np.cos(month)
...
# Split data
test_rows = pd.isnull(rawdata[pred_var])
...
print('CV log-loss: ', np.mean(scores), '+/-', np.var(scores))
